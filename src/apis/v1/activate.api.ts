...
activate_api.get('/admin/:user_number', async (req, res) => {
  try {
    const rawUserNumber = req.params.user_number
    const userToken = verifyExpToken<{ userNumber }>(rawUserNumber)
    if (userToken.userNumber) {
      const user = await UserModel.findOne({ userNumber: userToken.userNumber, userRole: EUserRole.ADMIN })
      if (!user) {
        throw Error('ไม่พบผู้ใช้')
      }
      const username = user.username
      if (user.isVerifiedEmail) {
        res.redirect(`https://admin.movematethailand.com/activate/exist/${username}`)
      } else {
        await user.updateOne({ status: EUserStatus.ACTIVE, isVerifiedEmail: true })
        await NotificationModel.sendNotification({
          userId: user._id,
        ...
        res.redirect(`https://admin.movematethailand.com/activate/success/${username}`)
      }
    } else {
      throw Error('รหัสผู้ใช้ไม่สมบูรณ์')
    }
  } catch (error) {
    res.redirect('https://admin.movematethailand.com/activate/error')
  }
})
