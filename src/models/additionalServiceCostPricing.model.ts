...
export class AdditionalServiceCostPricing extends TimeStamps {
  ...
  @Property({
    enum: EAdditionalServiceCostPricingUnit,
    default: EAdditionalServiceCostPricingUnit.PERCENT,
    required: true,
  })
  ...
  @Field({ nullable: true })
  get name(): string {
    const _additionalService = (get(this, '_doc.additionalService', undefined) || this.additionalService || undefined) as AdditionalService
    console.log('AdditionalServiceCostPricing name', _additionalService, this)
    if(_additionalService) {
        return _additionalService.name
    }
    return ''
  }

  static async getServicesPricing(
    ids: string[],
    costCalculation: boolean = false,
  ): Promise<{
    additionalServices: AdditionalServiceCostPricing[]
    ...
    const servicePrices = map<AdditionalServiceCostPricing, PriceItem>(additionalServices, (service) => {
      ...
      if (serviceName === 'POD') {
        return {
          label: `บริการคืนใบส่งสินค้า (${serviceName})`,
          price: service.price,
          cost: costCalculation ? service.cost : 0,
        }
      }
      return { label: serviceName, price: service.price, cost: costCalculation ? service.cost : 0 }
    })
    const { price, cost } = reduce(
      servicePrices,
      (prev, curr) => {
