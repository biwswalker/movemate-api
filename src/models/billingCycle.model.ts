    ...
    if (customer && customer.userType === EUserType.BUSINESS && customer.userRole === EUserRole.CUSTOMER) {
      ...
      if (businessDetail) {
        ...
        if (creditPayment) {
          ...
          const subTotalAmount = reduce(shipments, (prev, shipment) => {
            if (shipment.payment) {
...
export async function issueEmailToCustomer() {
  ...
  await Aigle.forEach(billingCycles, async (billingCycle) => {
    ...
    if (customer) {
      ((customer.businessDetail as BusinessCustomer).creditPayment as BusinessCustomerCreditPayment).financialContactEmails
...
export async function generateInvoice() {
  ...
  doc.font(sarabunLight).text('เลขที่ x ถนน x แขวง/ตำบล บางนา เขต/อำเภอ บางนา จังหวัด กรุงเทพมหานคร 1060', 110, doc.y - 9)
  ...
  doc.table({
    ...
    datas: range(1, 34).map((num) => ({ no: `${num}`, bookingDate: '1/8/2566', trackingNumber: 'MM1234567', detail: `ค่าขนส่ง${'รถกระบะตู้ทึบ'} ${'บางนา'} ไปยัง ${'ชลบุรี'}`, subtotal: '1,500.00', total: '1,500.00' }))
  }, {
    hideHeader: true,
  ...
  doc.fontSize(7).font(sarabunLight).text('( หนึ่งแสนเจ็ดหมื่นสามพันหกร้อยห้าสิบเอ็ดบาทเก้าสิบสี่สตางค์ )', 0, doc.y + 4, { align: 'right', width: 578 })

  // Policy detail
  doc.moveDown(3.5)
  doc.fontSize(8)
  doc.font(sarabunMedium).text('เงื่อนไขการชำระเงิน:', 22)
  doc.font(sarabunLight).text('ภายใน 7 วันปฏิทินนับจากวันที่ออกใบแจ้งหนี้ ในกรณีที่ชำระเงินไม่ตรงตามระยะเวลาที่กำหนด', 92, doc.y - 10)
  doc.moveDown(0.3)
  doc.font(sarabunLight).text('บริษัทฯจะคิดค่าธรรมเนียมอัตราร้อยละ 3.0 ต่อเดือนของยอดค้างชำระจนถึงวันที่ชำระเงินครบถ้วน ทั้งนี้ Movemate มีสิทธิ์ที่จะยกเลิกส่วนลดที่เกิดขึ้นก่อนทั้งหมด', 22)

  // Bank detail
  doc.moveDown(3.5)
  doc.font(sarabunLight).text('ช่องทางชำระ :')
  doc.font(sarabunLight).text('ธนาคาร กสิกรไทย', 80, doc.y - 10)
  doc.moveDown(0.5)
  doc.font(sarabunLight).text('ชื่อบัญชี :', 22)
  doc.font(sarabunLight).text('บริษัท เทพพรชัย เอ็นเทอร์ไพรส์ จำกัด', 80, doc.y - 10)
  doc.moveDown(0.5)
  doc.font(sarabunLight).text('เลขที่บัญชี :', 22)
  doc.font(sarabunLight).text('117-1-54180-4', 80, doc.y - 10)
  doc.moveDown(0.5)
  doc.font(sarabunLight).text('ประเภทบัญชี :', 22)
  doc.font(sarabunLight).text('ออมทรัพย์', 80, doc.y - 10)
  doc.moveDown(0.5)
  doc.font(sarabunLight).text('สาขา :', 22)
  doc.font(sarabunLight).text('เซ็นทรัล บางนา', 80, doc.y - 10)

  // After transfer detail
  doc.moveDown(1)
  doc.font(sarabunLight).fontSize(6).text('เมื่อท่ำนได้ชำระแล้วกรุณาส่งหลักฐานการชำระ มาที่ acc@movemateth.com พร้อมอ้างอิงเลขที่ใบแจ้งหนี้', 22)
  doc.moveDown(1)
  doc.font(sarabunLight).fontSize(6).text('*หากต้องการแก้ไขใบแจ้งหนี้และใบเสร็จรับเงิน กรุณาติตต่อ acc@movemateth.com ภายใน 3 วันทำการ')
  doc.moveDown(1)
  doc.font(sarabunLight).fontSize(6).text('หลังจากได้รับเอกสาร มิเช่นนั้นทำงบริษัทฯ จะถือว่ำเอกสำรดังกล่าวถูกต้อง ครบถ้วน สมบูรณ์ เป็นที่เรียบร้อยแล้ว')
