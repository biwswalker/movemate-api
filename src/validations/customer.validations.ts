...
const creditMethodValidation: any =
    (message: string) =>
        ([paymentMethod]: [string], schema: Yup.StringSchema<string>) => {
            return isEqual(paymentMethod, 'credit') ? schema.required(message) : schema.notRequired()
        }
...
const CreditPaymentSchema = Yup.object().shape({
    ...
    'financialContactEmails.0': Yup.string().when(
        ...
        ([paymentMethod, financialEmail], schema) => {
            // TODO: payment
            return isEqual(paymentMethod, 'credit')
    ...
    financialDistrict: Yup.string().when(
        'paymentMethod',
        creditMethodValidation('ระบุอำเภอ/แขวง ผู้ติดต่อด้านการเงิน'),
    ),
    ...
    financialSubDistrict: Yup.string().when(
        'paymentMethod',
        creditMethodValidation('ระบุตำบล/เขต ผู้ติดต่อด้านการเงิน'),
    ),
