...
export const IndividualDriverScema = (driverId?: string | undefined) =>
  Yup.object().shape({
    ...
    phoneNumber: Yup.string()
      ...
      .test('exiting-phonenumber', 'หมายเลขโทรศัพท์ถูกใช้งานแล้ว', async (value) => {
        const driver = driverId ? await UserModel.findById(driverId).lean() : undefined
        const existingDriver = await DriverDetailModel.findOne({
          phoneNumber: value,
          ...(driver && driver.driverDetail ? { _id: { $ne: driver.driverDetail } } : {}),
        })
...
export const BusinessDriverScema = (driverId?: string | undefined) =>
  Yup.object().shape({
    policyVersion: Yup.number(),
    driverType: Yup.string(),
...
export const EmployeeDriverScema = (driverId?: string | undefined) => Yup.object().shape({
  title: Yup.string().required('กรุณาเลือกคำนำหน้าชื่อ'),
  otherTitle: Yup.string().when('title', ([title], schema) =>
    isEqual(title, 'อื่นๆ') ? schema.required('ระบุคำนำหน้าชื่อ') : schema.notRequired(),
  ),
