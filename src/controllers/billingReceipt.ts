...
export async function generateBillingReceipt(billingId: string, sentEmail?: boolean, session?: ClientSession) {
  ...
  const _nonTaxReceipt = _isCashReceipt && _billing.amount.tax <= 0
  const { filePath, fileName, document } = _nonTaxReceipt
    ? await generateNonTaxCashReceipt(_billing)
  ...
  if (sentEmail) {
    ...
    await BillingDocumentModel.findByIdAndUpdate(document._id, { emailTime: new Date() }, { session })
  }
}
