...
import { CutomerIndividualInput, RegisterInput, UpdateUserInput } from "@inputs/user.input";
...
export default class RegisterResolver {
    ...
    async isExistingEmail(email: string, fieldName = 'email'): Promise<boolean> {
        if (email) {
            ...
            const isExistingEmailWithBusiness = await BusinessCustomerModel.findOne({ businessEmail: email });
    ...
    async register(
        ...
        try {
            ...
            // Exist email
            const email = isEqual(userType, 'individual') ? get(individualDetail, 'email', '') : isEqual(userType, 'business') ? get(businessDetail, 'businessEmail', '') : ''
            const emailFieldName = userType === 'individual' ? 'email' : 'businessEmail'
