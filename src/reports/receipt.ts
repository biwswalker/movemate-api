...
export async function generateReceipt(billingCycle: Billing, filname?: string): Promise<GenerateReceiptResponse> {
  ...
  return { fileName: '', filePath: '', document: null }
}

export async function generateNonTaxCashReceipt(
  billing: Billing,
  filname?: string,
  session?: ClientSession,
): Promise<GenerateReceiptResponse> {
  const _receipts = billing.receipts as Receipt[]
  const latestReceipt = last(sortBy(_receipts, 'createdAt')) as Receipt | undefined
  const fileName = filname ? filname : `receipt_${latestReceipt.receiptNumber}.pdf`
  const filePath = path.join(__dirname, '..', '..', 'generated/receipt', fileName)
  ...
  nomoredata = true
  await CashNoTaxReceiptFooterComponent(doc, billing)
  isOiginal = false
