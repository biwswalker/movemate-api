...
export function NonTaxRefundReceiptFooterComponent(doc: PDFDocument, refundNote: RefundNote) {
   ...
   * Signature
   */
  const issueBEDate = fDate(refundNote.refundDate, 'dd')
  const issueBEMonth = fDate(refundNote.refundDate, 'MM')
  const issueBEYear = toNumber(fDate(refundNote.refundDate, 'yyyy')) + 543
