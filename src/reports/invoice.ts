...
export async function generateInvoice(billing: Billing, session?: ClientSession): Promise<GenerateInvoiceResponse> {
  ...
  function HeaderComponent(page: number, totalPage: number, isOrigin: boolean = true) {
    // Logo
    ...
    doc.text(`ใบแจ้งหนี้ ${isOrigin ? '(ต้นฉบับ)' : '(สำเนา)'}`, 420, doc.y, { align: 'center', width: 162 })
    doc
      .lineCap('butt')
      .lineWidth(1)
      .moveTo(420, doc.y + 4)
  ...
  function ContentComponent(isOrigin: boolean = true) {
    if (isEmpty(shipmentGroup)) {
      HeaderComponent(1, 1, isOrigin)
    }
    ...
    forEach(shipmentGroup, (shipments, index) => {
      ...
      HeaderComponent(index + 1, shipmentGroup.length, isOrigin)
